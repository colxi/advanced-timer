...
(function(){
    ...
    const Timer =  function(t){
        ...
        const timer = {
            ...
            get timestamp(){
                let abstime;
                if( _timer_statusCode_=== -1 ) abstime= 0;
                else if( _timer_statusCode_=== 0 ) abstime = 0;
                else if( _timer_statusCode_=== 1 ) abstime = _time_ellapsed_ + ( new Date() - _time_lastCycle_ );
                else if( _timer_statusCode_=== 2 ) abstime = _time_ellapsed_ + ( _time_pause_ - _time_lastCycle_ );
                else if( _timer_statusCode_=== 3 ) abstime = _time_ellapsed_;
                return abstime || 0;
            },
            ...
            pause : function(){
                ...
                if( _timer_statusCode_ !== 1 ){
                    console.warn('Timer.start() Only running timers can be paused (Timer Status:'+_timer_statusCode_+')');
                    return this;
                }
             ...
             */
            resume: function(){
                if( _timer_statusCode_ === 0 ) return this.start();
                else if( _timer_statusCode_ !== 2 ){
                    console.warn('Timer.start() Only paused timers can be resumed (Timer Status:'+_timer_statusCode_+')');
                    return this;
                }
            ...
            repeat : function(c=true){
                if( (parseInt(c)!==c || c<1) && c!==true && c!==false) throw new Error('Timer.delay() expects an integer (>=1) or a boolean');
                ...
                if( _timer_statusCode_ === 1 || _timer_statusCode_ === 2 ){
                    if( _cycle_current_ >= _cycle_limit_ ) onComplete();
                }
                return this;
            },
             ...
             */
            reset: function(){
                if( _timer_statusCode_ === -1 ) throw new Error('');
